...
public class MySqlVoluntarioDAO implements VoluntarioDAO {
	...
	public int update(Voluntario bean) {
		...
		try {
			...
			// 5. Ejecutar ps
			// El método executeUpdate retorna el número de filas afectadas por la operación
			// de actualización.
			// Si todo está bien, debería retornar 1, lo que significa que una fila se
			// actualizó.
	...
	public List<Voluntario> findAll() {
		...
		try {
			...
			String sql = "SELECT v.*, i.eventos_id_evento, e.nombre AS especialidad_nombre, ev.nombre AS evento_nombre "
					+ "FROM voluntario v " + "LEFT JOIN inscripcion i ON v.dni = i.voluntario_dni "
	...
	public int deleteByEventId(int cod) {
		...
		try {
			...
			String sql = "DELETE v FROM voluntario v " + "INNER JOIN inscripcion i ON v.dni = i.voluntario_dni "
